...
namespace TeamPhoenix.MusiCali.Services
{
    public class CraftReceiptService
    {
        ...
        public async Task<bool> CreateAReceiptAsync( string buyerHash, string sku, bool offerable, decimal offerPrice, int quantity)
        {
            ...
            if (!(itemCreationDAO!.IsSkuDuplicate(sku)))
            {
                return false;
            ...
            string creatorHash = item.CreatorHash!;
            ...
            try
            {
                CraftReceiptModel newReceipt = new CraftReceiptModel( creatorHash, buyerHash, sku, item.Price, offerPrice, profit, revenue, quantity, offerable);
                ...
                if (offerable == false)         // if the receipt is offerable, dont reduce stock number. waiting for seller acceptance.
                {
                    bool deductStockAvailable = await itemCreationDAO.UpdateStockAvailable(sku, newstock);              
                }


                    messageToBuyer = "$Thank you for your interest in '{item.Name}'. The price for this item is ${item.Price}. For further information to make a purchase, please contact the seller at {sellerEmail}.";
                messageToSeller = "$Thank you for selling '{item.Name}'. The price for this item is ${item.Price}. For further information to finish a sale, please contact the buyer at {buyerEmail}.";
                SendConfirmationEmail(buyerEmail, "Sale Confirmation CreaftVerify", messageToBuyer);
                SendConfirmationEmail(sellerEmail, "Sale Confirmation CreaftVerify", messageToSeller);
