    ...
    public class ScaleDisplay
    {
        public bool ScaleDisplayLoggingService(string username)
        {
            try
            {
                ...
                var result = _logger.CreateLog(userHash, logLevel, logCategory, context); //calls the logger service to make the log
                return true;
            }
            catch(Exception ex){
                Console.WriteLine($"Error logging Scale Display: {ex.Message}");
                return false;
            }
        }
    }
